...
QUOTE_OPTIONAL_BACKSLASHES = re.compile(r"(?P<bs>(?:\\\\)*)" r'\\?(?P<quote>")')

ESCAPE_UNESCAPED_QUOTES = functools.partial(
    QUOTE_OPTIONAL_BACKSLASHES.sub, r"\g<bs>\\\g<quote>"
)
...
def a_list(label=None, kwargs=None, attributes=None):
    ...
    if kwargs:
        items = [
            "%s=%s" % (quote(k), quote(v))
            for k, v in tools.mapping_items(kwargs)
            if v is not None
        ]
        result.extend(items)
    if attributes:
...
def nohtml(s):
    ...
    try:
        ...
        raise TypeError(
            "%r does not have one of the required types:" " %r" % (s, list(NOHTML))
        )
    return subcls(s)
